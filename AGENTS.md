...
6. From that root, run `npm run render -- --source <model-directory> --out <output-directory>` after model changes. For a concrete project model, `<output-directory>` must be a durable, version-controlled location in that project repository; the rendered Context, Pulse, UI, and Deployment review artifacts must be saved there and committed with the accepted model change. Do not treat temporary render output outside the project repository as the durable review artifact.
7. From that root, start `npm run review -- --dir <output-directory>` against those repository-backed rendered artifacts and keep the local server running.
